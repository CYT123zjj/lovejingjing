...
uPasswd = 'tomato'
userList = [{'huloo': uPasswd}, {'tom': uPasswd}, {'athena': uPasswd}, {'jerry': uPasswd}]
gName = 'teachers'
num = 80
...
def check_file():
    ...
    if os.path.isfile('/var/tmp/fstab'):
        ...
        if not subprocess.Popen('getfacl /var/tmp/fstab | grep jerry ' , shell=True, stdout=subprocess.PIPE).stdout.readlines()[0] == 'jerry::rw-\n':
            print_res('ERROR: The file "/var/tmp/fstab" with "jerry" check error!!!' , num, -10)
            scoreDict['score'] -= 10
        if not subprocess.Popen('getfacl /var/tmp/fstab | grep tom ' , shell=True, stdout=subprocess.PIPE).stdout.readlines()[0] == 'tom::-\n':
            print_res('ERROR: The file "The file "/var/tmp/fstab" with "tom" check error!!!', num, -10)
            scoreDict['score'] -= 10
    else:
...
def check_cron():
    parting_line()
    print '8、检查计划任务'
    cronList=["30 10 * * *  echo 'haha'\n", "30 10 * * *  echo haha\n",'30 10 * * *  echo "haha"\n']
    cronNatasha = subprocess.Popen('crontab -l -u jerry', shell=True, stdout=subprocess.PIPE).stdout.readlines()[0]
    if cronNatasha not in cronList:
        print_res('ERROR: The crontab of jerry check error !!!', num, -10)
        scoreDict['score'] -=10
    return scoreDict['score']
...
def check_share_dir():
    parting_line()
    print '9、检查共享目录'
    if not os.path.isdir('/home/rhcedir'):
        print_res('ERROR: The dir "/home/rhcedir" check error !!!', num, -10)
        scoreDict['score'] -= 10
    else:
        if oct(os.stat('/home/rhcedir').st_mode)[2:] == '2770' and os.stat('/home/rhcedir').st_gid == 1001:
            print 'The dir permission "/home/rhcedir" check ok !!!             '
        else:
            print_res('ERROR: The dir permission "/home/rhcedir" check error !!!', num, -10)
            scoreDict['score'] -= 10
    return scoreDict['score']
def check_kernel():
    parting_line()
...
def check_stu_f():
    ...
    try:
        ...
        if os.path.isdir('/root/results'):
            fileList = subprocess.Popen('ls -A /root/results', shell=True, stdout=subprocess.PIPE).stdout.readlines()
            if fileList == []:
                print_res('ERROR: The  director "results" is empty!!!', num, -10)
                scoreDict['score'] -= 10
            else:
                print_ok('The files of student is found at "/root/results"')
                for i in fileList:
                    if os.stat('/root/results/%s' % i.strip()).st_uid != int(stuID):
            ...
            print_res('ERROR: Can`t find the user student OR the dir "results"!!!', num, -20)
...
def check_str():
    parting_line()
    print '16、检查字符串'
    if os.path.isfile('/root/wlist'):
        print_ok('Found the file "/root/wlist"')
        fList = subprocess.Popen(' grep tasia -v  /root/wlist', shell=True,stdout=subprocess.PIPE).stdout.readlines()
        if fList:
            print_res('ERROR: "%s" is not include "tasia"!!! ' %(fList[0].strip()), num, -10)
            scoreDict['score'] -= 10
        else:
            wlist = subprocess.Popen('cat /root/wlist', shell=True, stdout=subprocess.PIPE).stdout.readlines()
            words = subprocess.Popen('cat /usr/share/dict/words', shell=True, stdout=subprocess.PIPE).stdout.readlines()
            for eachStr in wlist:
        ...
        print_res('ERROR: The file "/root/wlist" is not found!!!', num, -20)
        scoreDict['score'] -= 20
    return scoreDict['score']
